 ...
 *   其他 :之后的其他位置
 *
 * 除了开始和目标之外，可以为事件设置出现率。
 * 在事件的备注内设置如下。
 * 不会生成不具有出现率设置的事件。
 *
 * -出现几率
 *   每个地图 :<RateMap: [Positive small number of less than 1.0]>
 *   每个房间 :<RateRoom:[Positive small number of less than 1.0]>
 *
 * -插件指令
 *   MapGenerator RoomAndPass # 生成有道路和房间的地图(地牢)
 *   MapGenerator FillRoom    # 生成一个用房间设置填充满的房间
 *
 * 没有插件参数
 *
 *
